...
def writeAllSpeciesTreeFastas(pamat,columnindex,familyindex,nucFasta,outdir,processes,fams):
    ...
    if len(os.listdir(outdir)) == len(familylist): #previously cached results
        return outdir
...
def nexusAlnSTreeFastas(sTreeDir):
...
#            with open('{}/{}.aln'.format(faadir,fout),'r') as fna, open('{}/{}.nex'.format(nexDir,fout),'w') as nex:
#                alignment = AlignIO.parse(fna,'fasta')
#                nexus = AlignIO.write(nex,'nexus')
    return nexDir

def checkTaxa(matrices):#from https://biopython.org/wiki/Concatenate_nexus
...
def main(pamat,columnindex,familyindex,nucFasta,outname,processes,fams):
    ...
    nexDir = nexusAlnSTreeFastas(fastadir)
