...
\begin{figure}[htb!]
    ...
    \begin{tikzpicture}
        ...
        \node [block,above=0.5cm of node4]  (node5)  {Genome Alignment};
...
Protein and nucleotide fastas of all CDS sequences and whole genome fastas for all bacterial strains included in this analysis were downloaded from NCBI RefSeq.
Only bacterial data from genomes annotated as complete by RefSeq were used.
Data was downloaded using custom scripts available in the github repo \verb|./scripts/scrapingData/{NCBINameScraper.py,get_gbffs.sh}|.
\ac{crsp} annotations of \ac{cas} and Cfp proteins from the \ac{crsp}one tool from Zhang and Ye were used to determine the presence of \ac{crsp} systems \citep{ineqcas}.
Data from their online results were parsed and downloaded using a custom python script available in the github  repo \verb|./scripts/scrapingData/crispr_one_scraper.py|
An \ac{otu} was annotated as having a \ac{crsp} system if it was annotated as having a \ac{crsp} system and at least 1 \ac{crsp} array sequence according to the CRISPROne database \cite{ineqcas}.
